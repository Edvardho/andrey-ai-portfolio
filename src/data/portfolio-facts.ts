...
const topicConfigs: Record<SynthesisTopic, SynthesisTopicConfig> = {
  identity: {
    ...
    subjectFacts: {
      candidate_motivation: unique([
        ...sharedFacts,
        'Андрей продолжает собирать и уточнять портфолио, чтобы показывать не только экраны, но и решения, доказательства и ограничения.',
        'В портфолио есть не только сильные кейсы вроде Альфа-Смарта и SIEBEL, но и честный слабый кейс ChatPoint.',
        'ChatPoint показывает, что Андрей готов разбирать собственные ошибки и не прятать неудачный продукт за красивой подачей.',
      ]),
    },
  ...
  experience: {
    ...
    subjectFacts: {
      design_system_work: unique([
        ...sharedFacts,
        'В Positive Technologies часть интерфейсов проектировалась с помощью готовой дизайн-системы.',
        'В Positive Technologies Андрей поддерживал UI-kit и вносил в него правки.',
        'В Альфа-Смарте команда работала внутри готовой дизайн-системы банка и ограничений BDUI.',
      ]),
    },
  ...
  web: {
    topic: 'web',
    title: 'Что Андрей делал в web',
    facts: unique([
      ...sharedFacts,
      'В web у Андрея есть B2B и внутренние продукты: SIEBEL, ChatPoint, web-часть Альфа-Смарта и интерфейсы в Positive Technologies.',
      'SIEBEL — web-интерфейс для операторов поддержки, где Андрей разбирал реальную работу операторов и перерабатывал workflow.',
      'ChatPoint — B2B-платформа коммуникации, где Андрей проектировал onboarding подключения каналов, routing, операторские и администраторские сценарии.',
      'Альфа-Смарт дошел до запуска на iOS, Android и web.',
      'В Positive Technologies Андрей работал с продуктовым UI и поддерживал UI-kit в рамках дизайн-системы.',
      ...caseAtomicFacts(siebel, { includeTakeaway: true, includeOutcomes: true }),
      ...caseAtomicFacts(chatpoint, { includeTakeaway: true, includeProof: true }),
      ...caseAtomicFacts(alfa, { includeOutcomes: true }),
    ]),
    fallbackTitle: 'Что он делал в web',
    fallbackParagraphs: [
      'В web у Андрея в первую очередь B2B и внутренние продукты: SIEBEL, ChatPoint, web-часть Альфа-Смарта и интерфейсы в Positive Technologies. Это не промо-лендинги, а рабочие сценарии с ролями, настройками, админками и передачей в разработку.',
      'В SIEBEL он разбирал операторский workflow, в ChatPoint проектировал onboarding, routing и админские сценарии, в Альфа-Смарте участвовал в web-версии подписки, а в Positive Technologies работал с продуктовым UI и дизайн-системой.',
    ],
    fallbackBullets: [],
    chips: [
      { id: 'facts-web-siebel', label: 'Открыть SIEBEL', action: { type: 'open_case_summary', caseId: 'siebel' } },
      { id: 'facts-web-chatpoint', label: 'Рассказать про ChatPoint', message: 'Расскажи про ChatPoint' },
      { id: 'facts-web-experience', label: 'Показать опыт работы', action: { type: 'open_experience_summary' } },
    ],
  },
  mobile: {
    topic: 'mobile',
    ...
    fallbackParagraphs: [
      'В мобильных кейсах у Андрея есть Альфа-Смарт, расходы держателей карт, шаринг подписки и UX/UI WannabeLike. Это разные задачи: банковская подписка, просмотр расходов семьи, короткий путь приглашения участника и учебный UI-концепт на данных.',
      'Его роль там — разбирать сценарии, роли и ограничения, собирать user flow, проектировать состояния и передавать понятные макеты в разработку. Самый сильный мобильный пример — Альфа-Смарт, потому что он дошел до релиза и метрик.',
    ],
    fallbackBullets: [],
    chips: [
  ...
  decision_making: {
    ...
    subjectFacts: {
      design_process: unique([
        ...sharedFacts,
        'В большинстве проектов Андрей проходил путь от исследования и структуры сценария до передачи решения в разработку или релиза.',
        'В Альфа-Смарте он разбирал требования, собирал user flow, готовил гипотезы, прототип для тестов, проходил дизайн-чек и передавал решение в разработку.',
        'В SIEBEL он начал не с редизайна по ощущениям, а с анализа реальной работы операторов и проверки решений через MVP.',
      ]),
      collaboration_process: unique([
        ...sharedFacts,
        'В Альфа-Смарте Андрей синхронизировался с бизнесом через user flow и ранние драфты, а затем передавал решение в разработку.',
        'В SIEBEL он согласовывал найденные проблемы с продактами и переводил их в решения для разработки.',
        'В расходах держателей главная сложность была межкомандной: нужно было договорить команды друг с другом и встроить сценарий в контур другой команды.',
      ]),
      stakeholder_feedback: unique([
        ...sharedFacts,
        'В Альфа-Смарте подтверждены дизайн-чек, аппрувы и дизайн-ревью после реализации.',
        'В SIEBEL подтверждено согласование проблем с продактами перед переводом их в продуктовые решения.',
        'Подробный процесс обработки обратной связи стейкхолдеров в портфолио описан не полностью.',
      ]),
      prioritization: unique([
        ...sharedFacts,
        'В Альфа-Смарте Андрей спорил за MVP отмены подписки в один экран вместо перегруженного retention-flow.',
        'В Альфа-Смарте после нескольких итераций команда ушла от web-first подхода к mobile-first сценарию.',
        'В SIEBEL фокус сместился с косметического редизайна на двухоконный режим, данные клиента и шаблоны ответов.',
        'В ChatPoint главный вывод связан с тем, что ценность продукта проверяли слишком поздно.',
      ]),
      ...
      learning_adaptation: unique([
        ...sharedFacts,
        'UX/UI WannabeLike появился потому, что Андрей хотел отдельно прокачать UI вне жестких ограничений банковской дизайн-системы.',
        'В этом кейсе подтверждены research synthesis, IA, user flows и UX/UI concept.',
        'В портфолио нет отдельного подробного списка всех случаев, где Андрею приходилось быстро учиться новому на боевом проекте.',
      ]),
    },
  ...
  delivery_evidence: {
    topic: 'delivery_evidence',
    title: 'Что портфолио подтверждает о доведении задач до результата',
    facts: unique([
      'В портфолио нет данных о дедлайнах, просрочках, оценке задач или личной дисциплине по срокам.',
      'В Альфа-Смарте Андрей разобрал требования, собрал user flow, подготовил гипотезы и прототип для теста, прошел дизайн-чек, передал решение в разработку и провел дизайн-ревью после реализации.',
      'Решение Альфа-Смарта дошло до запуска на iOS, Android и web.',
      'В SIEBEL исследование операторского workflow, решения и MVP дошли до релиза; среднее время обработки обращения сократилось с 900 до 580 секунд.',
    ]),
    subjectFacts: {
      behavioral_evidence_check: unique([
        'В портфолио нет данных о дедлайнах, просрочках, оценке задач или личной дисциплине по срокам.',
        'В Альфа-Смарте Андрей прошел путь от требований и user flow до дизайн-чека, handoff, дизайн-ревью после реализации и запуска на iOS, Android и web.',
        'В SIEBEL исследование реальной работы операторов, решения и MVP дошли до релиза; среднее время обработки обращения сократилось с 900 до 580 секунд.',
      ]),
    },
    fallbackTitle: 'Что можно сказать о сроках и исполнительности',
    fallbackParagraphs: [
    ...
    fallbackBullets: [],
    chips: [
      { id: 'facts-delivery-alfa', label: 'Где это видно?', message: 'Где доказательства по Альфа-Смарту?' },
      { id: 'facts-delivery-interview', label: 'Почему его стоит звать?', message: 'Почему его стоит звать на интервью?' },
      { id: 'facts-delivery-contact', label: 'Написать Андрею', action: { type: 'open_contact_modal', source: 'facts:delivery' } },
    ],
  },
  risks: {
    topic: 'risks',
